...
public class PlanetQuiz : MonoBehaviour
{
    ...
    private IEnumerator CorrectRoutine()
    {
        ...
        if (AnswerTrigger.Instance != null)
        {
            ...
            if (currentPlanetVisual.planetName == correctAnswer && currentPlanetVisual.infoPanel != null)
            {
                currentPlanetVisual.infoPanel.SetActive(true);
                
                if (currentPlanetVisual.descriptionText != null)
                    currentPlanetVisual.descriptionText.text = currentPlanetVisual.model.GetComponent<PlanetVisual>().description;

                // [THÊM MỚI] Cho phép xoay hành tinh TO
                if (PlanetRotator.Instance != null)
                    PlanetRotator.Instance.SetPlanet(currentPlanetVisual.model);

                // [THÊM MỚI] Dừng script tự xoay để đứng yên cho người dùng xoay thủ công
                PlanetOrbit orbit = currentPlanetVisual.GetComponentInParent<PlanetOrbit>();
            ...
            AnswerTrigger.Instance.PlayDescription(correctAnswer, () =>
            {
                ...
                if (currentPlanetVisual.planetName == correctAnswer && currentPlanetVisual.infoPanel != null)
                    currentPlanetVisual.infoPanel.SetActive(false);
